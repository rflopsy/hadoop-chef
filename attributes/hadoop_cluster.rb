...
default[:hadoop][:cdh_version]   = 'cdh4.2.1'
...
default[:hadoop][:hadoop_daemon_heapsize]            = 2000

#
# fs.inmemory.size.mb  # default XX
#

default[:groups]['hadoop'    ][:gid] = default[:owners]['hadoop'    ][:uid] = 300
default[:groups]['supergroup'][:gid] = default[:owners]['supergroup'][:uid] = 301
default[:groups]['hdfs'      ][:gid] = default[:owners]['hdfs'      ][:uid] = 302
default[:groups]['mapred'    ][:gid] = default[:owners]['mapred'    ][:uid] = 303
...
if node[:ec2]
  hadoop_performance_settings =
    ...
    when 'm2.4xlarge' then { :max_map_tasks => 7, :max_reduce_tasks => 4, :java_child_opts => '-Xmx4378m -XX:+UseCompressedOops -XX:MaxNewSize=200m -server', :java_child_ulimit => 13447987, :io_sort_factor => 40, :io_sort_mb => 256, }
    else
